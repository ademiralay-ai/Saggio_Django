(function () {
    ...
    const el = {
        ...
        commandWrap: document.getElementById("rcc-command-wrap"),
        ...
        agentAdminBody: document.getElementById("rcc-agent-admin-body"),
        agentFormMode: document.getElementById("rcc-agent-form-mode"),
        agentNewBtn: document.getElementById("rcc-agent-new-btn"),
        genTokenBtn: document.getElementById("rcc-gen-token-btn"),
    ...
    function setAgentFormMode() {
        const isEdit = Boolean(state.editingAgentCode);
        if (el.agentFormMode) {
            el.agentFormMode.textContent = isEdit
                ? `Mod: Düzenleme (${state.editingAgentCode}) - Token girmezsen değişmez`
                : "Mod: Yeni robot";
        }
        if (el.agentNewCode) {
            el.agentNewCode.readOnly = isEdit;
        }
    }

    function resetAgentForm() {
        state.editingAgentCode = "";
        if (el.agentNewCode) el.agentNewCode.value = "";
        if (el.agentNewName) el.agentNewName.value = "";
        if (el.agentNewToken) el.agentNewToken.value = "";
        if (el.agentEnabled) el.agentEnabled.value = "1";
        if (el.agentDesired) el.agentDesired.value = "";
        setAgentFormMode();
    }

    function fillAgentForm(code) {
        const row = state.agents.find((a) => String(a.code) === String(code));
        if (!row) return;
        state.editingAgentCode = String(row.code || "");
        if (el.agentNewCode) el.agentNewCode.value = String(row.code || "");
        if (el.agentNewName) el.agentNewName.value = String(row.name || "");
        if (el.agentNewToken) el.agentNewToken.value = "";
        if (el.agentEnabled) el.agentEnabled.value = row.is_enabled ? "1" : "0";
        if (el.agentDesired) el.agentDesired.value = String(row.desired_version || "");
        setAgentFormMode();
        if (el.agentNewName && typeof el.agentNewName.focus === "function") {
            el.agentNewName.focus();
        }
    }

    function renderAgentAdminTable(rows) {
        if (!el.agentAdminBody) return;
        if (!rows || !rows.length) {
            el.agentAdminBody.innerHTML = '<tr><td colspan="9" class="empty-cell">Robot kaydı yok.</td></tr>';
            return;
        }
        const html = rows.map((a) => {
            const status = String(a.status || "offline").toLowerCase();
            const enabledVal = a.is_enabled ? "1" : "0";
            return `
                <tr>
                    <td>${esc(a.code || "-")}</td>
                    <td><input type="text" class="rcc-inline-input" data-agent-name="${esc(a.code)}" value="${esc(a.name || "")}"></td>
                    <td class="rcc-status-cell ${esc(status)}">${esc(status)}</td>
                    <td>
                        <select class="rcc-inline-input" data-agent-enabled="${esc(a.code)}">
                            <option value="1" ${enabledVal === "1" ? "selected" : ""}>Aktif</option>
                            <option value="0" ${enabledVal === "0" ? "selected" : ""}>Pasif</option>
                        </select>
                    </td>
                    <td>${esc(a.ip_address || "-")}</td>
                    <td>${esc(a.agent_version || "-")}</td>
                    <td><input type="text" class="rcc-inline-input" data-agent-desired="${esc(a.code)}" value="${esc(a.desired_version || "")}" placeholder="1.3.1"></td>
                    <td>
                        <div class="rcc-token-wrap">
                            <input type="text" class="rcc-inline-input" data-agent-token="${esc(a.code)}" placeholder="Opsiyonel yeni token">
                            <button class="btn btn-sm" data-gen-token="${esc(a.code)}" title="Otomatik oluştur"><i class="fas fa-random"></i></button>
                        </div>
                    </td>
                    <td>
                        <div class="rcc-inline-actions">
+                            <button class="btn btn-sm" data-save-agent="${esc(a.code)}" title="Kaydet"><i class="fas fa-floppy-disk"></i></button>
+                            <button class="btn btn-sm" data-disable-agent="${esc(a.code)}" title="Pasifleştir"><i class="fas fa-power-off"></i></button>
+                            <button class="btn btn-sm" data-delete-agent="${esc(a.code)}" title="Sil"><i class="fas fa-trash"></i></button>
                        </div>
                    </td>
                </tr>
            `;
        }).join("");
        el.agentAdminBody.innerHTML = html;

        const getRowValues = (code) => {
            const nameInput = el.agentAdminBody.querySelector(`[data-agent-name="${CSS.escape(code)}"]`);
            const enabledSel = el.agentAdminBody.querySelector(`[data-agent-enabled="${CSS.escape(code)}"]`);
            const desiredInput = el.agentAdminBody.querySelector(`[data-agent-desired="${CSS.escape(code)}"]`);
            const tokenInput = el.agentAdminBody.querySelector(`[data-agent-token="${CSS.escape(code)}"]`);
            return {
                code: String(code || "").trim(),
                name: String(nameInput?.value || "").trim(),
                is_enabled: String(enabledSel?.value || "1") === "1",
                desired_version: String(desiredInput?.value || "").trim(),
                token: String(tokenInput?.value || "").trim(),
                tokenInput: tokenInput,
            };
        };

        el.agentAdminBody.querySelectorAll("[data-save-agent]").forEach((btn) => {
            btn.addEventListener("click", async () => {
                const code = btn.getAttribute("data-save-agent") || "";
                const payload = getRowValues(code);
                if (!payload.code || !payload.name) {
                    showToast("Kod ve ad zorunlu", "error");
                    return;
                }
                btn.disabled = true;
                try {
                    await postJson(urls.upsert, {
                        code: payload.code,
                        name: payload.name,
                    ...
                    if (payload.tokenInput) payload.tokenInput.value = "";
                    showToast(`Ajan güncellendi: ${payload.code}`, "success");
                    await refreshAgents();
                } catch (err) {
                    showToast("Ajan güncelleme hatası: " + err.message, "error");
                } finally {
                    btn.disabled = false;
                }
            });
        ...
        el.agentAdminBody.querySelectorAll("[data-disable-agent]").forEach((btn) => {
            btn.addEventListener("click", async () => {
                const code = btn.getAttribute("data-disable-agent") || "";
                const payload = getRowValues(code);
                if (!payload.code || !payload.name) {
                    showToast("Kod ve ad zorunlu", "error");
                    return;
                }
                if (!window.confirm(`${payload.code} pasifleştirilsin mi?`)) return;
                btn.disabled = true;
                try {
                    await postJson(urls.upsert, {
                        code: payload.code,
                    ...
                    if (payload.tokenInput) payload.tokenInput.value = "";
                    showToast(`Ajan pasifleştirildi: ${payload.code}`, "success");
                    await refreshAgents();
                } catch (err) {
                    showToast("Pasifleştirme hatası: " + err.message, "error");
                } finally {
                    btn.disabled = false;
                }
            });
        ...
        el.agentAdminBody.querySelectorAll("[data-delete-agent]").forEach((btn) => {
            btn.addEventListener("click", async () => {
                const code = btn.getAttribute("data-delete-agent") || "";
                if (!code) return;
                if (!window.confirm(`${code} silinsin mi? Bu işlem geri alınamaz.`)) return;
                btn.disabled = true;
                try {
                    await postJson(urls.deleteAgent, { code: code });
                    showToast(`Ajan silindi: ${code}`, "success");
                    if (state.editingAgentCode === code) {
                        resetAgentForm();
                    }
                    await refreshAll();
                } catch (err) {
        ...
        el.agentAdminBody.querySelectorAll("[data-gen-token]").forEach((btn) => {
            btn.addEventListener("click", () => {
                const code = btn.getAttribute("data-gen-token") || "";
                const inp = el.agentAdminBody.querySelector(`[data-agent-token="${CSS.escape(code)}"]`);
                if (inp) inp.value = genToken();
            });
        });
    }
    ...
    function renderJobs(rows) {
        ...
        const html = rows.map((j) => {
            ...
            const label = j.target_agent_code
                ? `${j.target_agent_code}${j.target_agent_ip ? " (" + j.target_agent_ip + ")" : ""}`
                : "(otomatik)";
    ...
    function handleCommandTypeChange() {
        ...
        if (el.commandWrap) {
            el.commandWrap.style.display = type === "run_command" ? "" : "none";
        }
    }
    ...
    async function onDispatchSubmit(e) {
        ...
        if (commandText && commandType === "run_command") {
    ...
    async function onAgentSubmit(e) {
        e.preventDefault();
        const isEditMode = Boolean(state.editingAgentCode);
        const body = {
        ...
        if (!body.code || !body.name) {
            showToast("Ajan kodu ve adı zorunlu", "error");
            return;
        }
        if (!isEditMode && !body.token) {
            showToast("Yeni robot için token zorunlu", "error");
            return;
        }
    ...
    function init() {
        ...
        if (el.agentNewBtn) el.agentNewBtn.addEventListener("click", resetAgentForm);
        if (el.genTokenBtn) el.genTokenBtn.addEventListener("click", () => {
            if (el.agentNewToken) el.agentNewToken.value = genToken();
        });
